//L函数
//直接计算，不用再走一遍K函数了
function funcL_local(Funcdata, r, ind) {
    //Funcdata是所有的数据，r是半径，ind是要计算的线段ID
    let L = 0;
    let Upper = 0;
    for (let i = 0; i < Funcdata.length; i++) {
        let sita = 0;
        if (FlowDis(Funcdata[ind], Funcdata[i]) <= r && i != ind)
            sita = 1;
        Upper += sita;
    }
...
function Is_Within_r(Funcdata,r,ind,top10){
    //使用的是二维距离
    let label=false;
    ...
    for(let i=0;i<top10.length;i++){
        let dis=FlowDis(Funcdata[top10[i]],Funcdata[ind]);
        if(dis<=r)
            label=true;
    }
    return label;
...
function cal_label(Funcdata, r) {
    ...
    let top10=[];
    for(let i=0;i<Funcdata.length;i++){
        let localL=funcL_local(Funcdata,r,i);
        Llist.push(localL);
    ...
    for(let i=0;i<Funcdata.length;i++){
        if(Is_Within_r(Funcdata,r,i,top10))
            Labeldata.push(1);
        else
            Labeldata.push(0);
    }
